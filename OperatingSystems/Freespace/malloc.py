...
# 1. It is predictable if given:
#    base address = 1000
#    Heap size = 100
#    The free list also gradually has a smaller size of free space block.

# 2. Worst fit uses the largest free memory block and the small black was 
#    not used until the larger memory blocks are not used.

# 3. Since all the available slots don't have to be examined, the look
#    for empty space speeds up. The first fit memory block is allocated to 
#    the job requestion memory in heap.

# 4. SIZESORT+ speeds up BEST, SIZESORT- makes memory allocation time O(1) for WORST,
#    SIZESORT- is the fastest for FIRST

# 5. Without coalescing larger allocation requests failed in free memory 
#    and the free list becomes conjested with a bunch of small free memory blocks
#    which also increases the look up time.

# 6. When the percent allocated fraction is near 100 or 100 almost all operations are 
#    allocated operations. When it nears 0 the allocations and free operations are 
#    done in 50% because you cannot free memory that isn't allocated.
