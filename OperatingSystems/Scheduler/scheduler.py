...
if options.solve == True:
    print ('** Solutions **\n')
    # 6. With SJF if the job length keeps increasing teh jobs which get executed after the running jobs will have to 
    # wait. It will continue to execute till the completeion of a job and the remaining jobs will keep waiting.
    if options.policy == 'SJF':
        joblist = sorted(joblist, key=operator.itemgetter(1))
        options.policy = 'FIFO'
    # 4. SJF and FIFO have the same turnaround time when the jobs arrive in increasing order of their length
    ...
    # 5. When the time quantum is larger than the largest job and the jobs arrive in increasing order of their workloads 
    # then RR will have the same response time as SJF.
